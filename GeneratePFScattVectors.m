function [q_pf,eqqdata] = GeneratePFScattVectors(th, eta, ome, hkl)
...
q_pf    = zeros(n_eta * n_ome,3,length(hkl));
eqqdata    = zeros(n_eta * n_ome,1,length(hkl));
for jj = 1:length(hkl)
    q0  = [1 0 0]';
    %y-axis rotation
    R_th    = [ ...
...
%         fname   = [ImageNames{i,1}, '.lattprms.mat'];
%         pfname  = fullfile(LSPath, fname);
%         tempdata = load(pfname);
%         tempdata = tempdata.eqq(:,jj);
%         eqqdata(ini:fin,:,jj)  = tempdata;
    end
end
